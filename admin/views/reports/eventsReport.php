...
class PDF extends FPDF
{
    function LoadData()
    {
        ...
        foreach ($evento->Listar() as $r) {
            $data[] = array($r->event_id, $r->name, $r->description, $r->venue, $r->date);
        }
        //print_r($data);
        return $data;
    }
    function header()
    {
        ...
        $this->Cell(260, 50, 'Registro de eventos', 0, 0, 'C');
        $this->Ln();
...
$pdf->AliasNbPages();
$pdf->AddPage('L', 'A4', 0);
$header  = array('Id', 'Nombre', 'Descripcion', 'Lugar', 'Fecha');
$data = $pdf->LoadData();
